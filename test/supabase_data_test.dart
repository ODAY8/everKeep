...
void main() {
  ...
  group('DocumentService', () {
    ...
    test('fetch reads the documents table newest first and maps rows', () async {
      supabase.route = (_) => jsonResponse([
        docRow(id: 'a', verified: true, filePath: '$testUserId/documents/a.pdf'),
    ...
    test('add sends only the writable columns — never an owner or a verified flag',
        () async {
      supabase.route = (_) => jsonResponse(docRow(id: 'new', title: 'Passport.pdf', age: 0));

      final saved = await service.addDocument(
        const DocumentItem(id: '', title: '  Passport.pdf ', subtitle: '', category: 'Legal'),
      );
    ...
    test('add with a file uploads to the user\'s own private folder first',
        () async {
      ...
      await service.addDocument(
        const DocumentItem(id: '', title: 'Scan', subtitle: '', category: 'Legal'),
      ...
      expect(upload.path, startsWith('/storage/v1/object/documents/$testUserId/documents/'));
    ...
    test('if saving the row fails, the uploaded file is removed again',
        () async {
      ...
      expect(supabase.where('DELETE', '/storage/v1/object/documents'), hasLength(1));
    });

    test('delete removes the stored file before the row', () async {
      supabase.route = (request) {
        if (request.method == 'GET') {
          return jsonResponse([{'file_path': '$testUserId/documents/a.pdf'}]);
        }
        return jsonResponse([]);
      };
      ...
      expect(supabase.requests.map((r) => '${r.method} ${r.path.split('/')[1]}'), [
        'GET rest',
        'DELETE storage',
        'DELETE rest',
      ]);
    ...
    test('if the file can\'t be removed, the row is kept so delete can be retried',
        () async {
      supabase.route = (request) {
        if (request.method == 'GET') {
          return jsonResponse([{'file_path': '$testUserId/documents/a.pdf'}]);
    ...
    test('deleting a document without a file never touches Storage', () async {
      supabase.route = (request) =>
          request.method == 'GET' ? jsonResponse([{'file_path': null}]) : jsonResponse([]);
    ...
    test('download links are short-lived signed URLs, not public URLs', () async {
      supabase.route = (_) => jsonResponse({'signedURL': '/object/sign/documents/a.pdf?token=abc'});

      final url = await service.createDownloadUrl('$testUserId/documents/a.pdf');

      final request = supabase.single('POST', '/storage/v1/object/sign/documents/');
      expect((request.json as Map)['expiresIn'], 300);
      expect(url, contains('token=abc'));
    });

    test('a database rejection becomes a safe message that leaks nothing',
        () async {
      supabase.route = (_) => rlsViolation();

      final error = await failureOf(
        () => service.addDocument(
          const DocumentItem(id: '', title: 'x', subtitle: '', category: 'Legal'),
        ),
      );
  ...
  group('AccountService', () {
    ...
    test('fetch maps rows, deriving icon, color and subtitle', () async {
      supabase.route = (_) => jsonResponse([
        ...
        accountRow(id: 'acc-2', name: 'Bank', username: null, category: 'Banking'),
      ]);

      final accounts = await service.fetchAccounts();

      expect(supabase.single('GET', '/rest/v1/accounts').query['order'], startsWith('created_at.desc'));
    ...
    test('add sends the descriptive fields only — there is no secret to send',
        () async {
      ...
      final body = supabase.single('POST', '/rest/v1/accounts').json as Map<String, dynamic>;
      expect(body, {
        'name': 'GitHub',
        'username': 'alex@dev.io',
        'category': 'Work',
    ...
    test('update targets the row by id and returns the saved values', () async {
      ...
      expect(request.json, {'name': 'GitHub Pro', 'username': null, 'category': 'Work'});
      expect(saved.title, 'GitHub Pro');
    });

    test('updating an account that isn\'t there fails instead of pretending',
        () async {
      supabase.route = (_) => jsonResponse([]); // RLS hides it, or it was deleted
    ...
    test('setFavorite sends the explicit value for that one row', () async {
      supabase.route = (_) => jsonResponse([{'id': 'acc-1'}]);
    ...
    test('a favorite that matched no row is a failure (so the UI rolls back)',
        () async {
      supabase.route = (_) => jsonResponse([]);
    ...
    test('delete targets the row by id', () async {
      ...
      expect(supabase.single('DELETE', '/rest/v1/accounts').query['id'], 'eq.acc-1');
    });
  });
  ...
  group('TrustedContactService', () {
    ...
    test('fetch reads oldest first; a missing avatar becomes empty', () async {
      supabase.route = (_) => jsonResponse([contactRow(), contactRow(id: 'tc-2', avatar: 'a.png')]);

      final contacts = await service.fetchContacts();

      expect(supabase.single('GET', '/rest/v1/trusted_contacts').query['order'], startsWith('created_at.asc'));
      expect(contacts.first.avatarUrl, '');
      expect(contacts.last.avatarUrl, 'a.png');
      expect(contacts.first.accessLevel, 'Full Access');
    });

    test('add sends the fields, not an owner, and returns the saved row',
        () async {
      ...
      final body = supabase.single('POST', '/rest/v1/trusted_contacts').json as Map;
      expect(body, {
        'name': 'Sarah Johnson',
        'relationship': 'Spouse',
        'access_level': 'Full Access',
    ...
    test('remove deletes by id', () async {
      ...
      expect(supabase.single('DELETE', '/rest/v1/trusted_contacts').query['id'], 'eq.tc-1');
    });
  ...
  group('SettingsService', () {
    ...
    test('fetch reads only the signed-in user\'s row', () async {
      ...
      expect(supabase.single('GET', '/rest/v1/security_settings').query['user_id'], 'eq.$testUserId');
    ...
    test('update upserts the user\'s single row', () async {
      ...
      await service.updateSecuritySettings(
        const SecuritySettings(twoFactorEnabled: true, loginAlertsEnabled: true),
      );
  ...
  group('VaultService', () {
    test('builds the summary from the user\'s real data', () async {
      ...
      final summary = await VaultServiceImpl(client: supabase.client).fetchVaultSummary();

      expect(summary.documentsCount, 3);
      expect(summary.passwordsCount, 2);
      expect(summary.totalItems, 5);
      expect(summary.storageUsedMb, closeTo(3.0, 0.001));
      expect(summary.securityScore, 67); // 2 of 3 protections on
      expect(summary.securityScoreLabel, 'Good — 67/100');
      expect(summary.legacyProgress, 1.0); // document, account, contact, setting
      ...
      expect(summary.financialsCount, 0);
    });
  });
  ...
  group('UserService', () {
    ...
    test('fetch combines the profile row with the Auth identity', () async {
      supabase.route = (_) => jsonResponse([
        {'id': testUserId, 'full_name': 'Alexandra Rivera', 'phone': '555-0100', 'avatar_url': null},
      ]);

      final user = await service.fetchUserProfile();

      expect(supabase.single('GET', '/rest/v1/profiles').query['id'], 'eq.$testUserId');
      expect(user.name, 'Alexandra Rivera'); // profile wins over sign-up metadata
      expect(user.phone, '555-0100');
      expect(user.email, 'alex@example.com'); // identity comes from Auth
      expect(user.id, testUserId);
    });

    test('a missing profile row is created rather than leaving the account without one',
        () async {
      supabase.route = (request) => request.method == 'GET'
          ? jsonResponse([])
          : jsonResponse({'id': testUserId, 'full_name': 'Alex Rivera', 'phone': null, 'avatar_url': null});
    ...
    test('update writes the editable fields and never the email', () async {
      ...
      final saved = await service.updateUserProfile(
        const User(id: testUserId, name: ' Jane Doe ', email: 'someone@else.com', phone: '  '),
      );

      final request = supabase.single('PATCH', '/rest/v1/profiles');
      expect(request.query['id'], 'eq.$testUserId');
      expect(request.json, {'full_name': 'Jane Doe', 'phone': null, 'avatar_url': null});
  ...
  group('without a session', () {
    test('services refuse to run and make no request', () async {
      ...
      final error = await failureOf(
        () => SettingsServiceImpl(client: signedOut.client).fetchSecuritySettings(),
      );

      expect(error.toString(), 'Your session has expired. Please sign in again.');
